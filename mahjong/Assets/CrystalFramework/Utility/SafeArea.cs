...
namespace Crystal
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        void Awake ()
        {
            ...
            CameraAspectRatioAdjuster = MainCamera.GetComponent<CameraAspectRatioAdjuster>();
        ...
        void ApplySafeArea (Rect r)
        {
            ...
            if (Screen.width > 0 && Screen.height > 0)
            {
                ...
                if (!CameraAspectRatioAdjuster.isSafeAreaInViewport)
                {
                    anchorMin.x = Mathf.Max(Camera.rect.x, anchorMin.x);
                    anchorMin.y = Mathf.Max(Camera.rect.y, anchorMin.y);
                    anchorMax.x = Mathf.Min(1 - Camera.rect.x, anchorMax.x);
                    anchorMax.y = Mathf.Min(1 - Camera.rect.y, anchorMax.y);
                }
